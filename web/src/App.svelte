<script lang="ts">
  ...
  import { asyncRun } from "./lib/python/pyoide";
  ...
  function initialiseMap(_: HTMLElement) {
    ...
    map = new maplibregl.Map({
      container: "map",
      style:
        "https://api.maptiler.com/maps/uk-openzoomstack-light/style.json?key=g6kCkRKHQMJqJMcThytt",
...
test= files('demoland_engine.test')

with open(test.joinpath("hgb.pickle"), "rb") as f:
    est = pickle.load(f)
...
print(prediction)
  ...
  function enableClick(featureId: number) {
    map.setFeatureState({ source: SOURCE_ID, id: featureId }, { click: true });
  }
  ...
  function updateMapData(mapData: GeoJSON.FeatureCollection) {
    if (map) {
      (map.getSource(SOURCE_ID) as maplibregl.GeoJSONSource).setData(mapData);
      updateLayers();
    }
  }
