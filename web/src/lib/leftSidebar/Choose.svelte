<script lang="ts">
  ...
  function decreaseCompareScenario() {
    const index = allCompareScenariosExceptMain.indexOf($compareScenarioName);
    if (index > 0) {
  ...
  $: {
    ...
    increaseCompareScenarioOk =
      $compareScenarioName !==
      allCompareScenariosExceptMain[allCompareScenariosExceptMain.length - 1];
  }
</script>
...
<div class="controls-grid">
  ...
  <button class="toggle-description" on:click={toggleDescriptionVisible}>
    <span class="instruction"
      >{descriptionVisible ? "hide description" : "show description"}</span
    >
